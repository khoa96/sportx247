<?php include "templates/head.php"; ?>

<body>
  <?php include "templates/header.php"; ?>
  <?php include "templates/subHeader.php"; ?>

  <!-- MAIN CONTENT -->
  <div class="home-page__wrapper">
    <div class="container">
      <div class="home-page__content">
        <aside class="home-page__aside-left">
          <section class="common-left-section">
            <div class="common-left-section__header">
              <h2 class="common-left-section__title">Football</h2>
            </div>
            <div class="common-left-section__list">
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-1.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Premier League Premier LeaguePremier LeaguePremier
                  LeaguePremier LeaguePremier League</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-2.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Bundesliga Premier LeaguePremier LeaguePremier
                  LeaguePremier League</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-3.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Ligue 1</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-4.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">La Liga</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-5.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Serie A</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-6.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">V-League</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-7.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Champions League</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-8.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Europa League</p>
              </a>
            </div>
          </section>
          <section class="common-left-section">
            <div class="common-left-section__header">
              <h2 class="common-left-section__title">All leagues</h2>
              <form action="" method class="common-left-section__form-search">
                <div class="form-search-item__wrapper">
                  <svg xmlns="http://www.w3.org/2000/svg" width="19" height="20" viewBox="0 0 19 20" fill="none">
                    <path
                      d="M15.4377 16.2497L12.6668 13.333M14.2502 8.74967C14.2502 11.7412 11.9463 14.1663 9.10433 14.1663C6.26236 14.1663 3.9585 11.7412 3.9585 8.74967C3.9585 5.75813 6.26236 3.33301 9.10433 3.33301C11.9463 3.33301 14.2502 5.75813 14.2502 8.74967Z"
                      stroke="#1A202C" stroke-linecap="round" stroke-linejoin="round" />
                  </svg>
                  <input type="text" name="" value="" placeholder="Search" class="form-search-item__input">
                </div>
              </form>
            </div>
            <div class="common-left-section__list">
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-1.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Premier League</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-2.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Bundesliga</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-3.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Ligue 1</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-4.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">La Liga</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-5.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Serie A</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-6.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">V-League</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-7.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Champions League Premier LeaguePremier LeaguePremier
                  League</p>
              </a>
              <a href="#" title="" target="" class="common-left-section__item">
                <img src="./images/sport-league-8.png" class="common-left-section__image" alt="">
                <p class="common-left-section__name text-overflow">Europa League Premier LeaguePremier LeaguePremier
                  LeaguePremier League</p>
              </a>
            </div>
            <a href="#" target="" title="" class="btn-see-all">See All</a>
          </section>
        </aside>
        <aside class="home-page__aside-right">
					<!-- LIVE MATCH -->
          <section class="common-section-match section-hot-match">
            <div class="common-section-match__header">
              <img src="./images/icon-radian.png" class="common-section-match__header-image" alt="">
              <h3 class="common-section-match__header-title">LIVE</h3>
            </div>
            <div class="common-section-match__body">
              <div class="common-section-match__list">
                <a class="common-section-match__item">
                  <div class="column-40 league-column">
                    <div class="first-column">
                      <div class="status-icon">
                        <iframe
                          src="https://lottie.host/embed/5290d45d-a6f7-4e7f-8dec-6fd4666e4c3f/VHdTOAPywV.json"></iframe>
                      </div>
                      <span class="status-text">live</span>
                    </div>
                    <div class="middle-column">
                      <p class="time-label__content">2nd half 52'</p>
                    </div>
                    <div class="last-column">
                      <p class="league-name text-overflow">Premier League nguyen dang khoa</p>
                    </div>
                  </div>
                  <div class="column-60 match-column">
                    <div class="clubs-wrapper">
                      <div class="club__item first-club">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester United United nguyen dang khoa nguye United
                            nguyen dang khoa nguye</p>
                        </div>
                        <img src="./images/sport-league-1.png" class="club-image" alt="">
                      </div>
                      <div class="result-score">1 - 2</div>
                      <div class="club__item second-club">
                        <img src="./images/sport-league-2.png" class="club-image" alt="">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester United nguyen dang khoa nguyen United nguyen
                            dang khoa nguye</p>
                        </div>
                      </div>
                    </div>
                    <button class="btn-watch">Watch</button>
                  </div>
                </a>
                <a class="common-section-match__item">
                  <div class="column-40 league-column">
                    <div class="first-column">
                      <div class="status-icon">
                        <iframe
                          src="https://lottie.host/embed/5290d45d-a6f7-4e7f-8dec-6fd4666e4c3f/VHdTOAPywV.json"></iframe>
                      </div>
                      <span class="status-text">live</span>
                    </div>
                    <div class="middle-column">
                      <p class="time-label__content">1nd half 2'</p>
                    </div>
                    <div class="last-column">
                      <p class="league-name text-overflow">Premier League </p>
                    </div>
                  </div>
                  <div class="column-60 match-column">
                    <div class="clubs-wrapper">
                      <div class="club__item first-club">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester</p>
                        </div>
                        <img src="./images/sport-league-3.png" class="club-image" alt="">
                      </div>
                      <div class="result-score">1 - 2</div>
                      <div class="club__item second-club">
                        <img src="./images/sport-league-4.png" class="club-image" alt="">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester United</p>
                        </div>
                      </div>
                    </div>
                    <button class="btn-watch">Watch</button>
                  </div>
                </a>
								 <a class="common-section-match__item">
                  <div class="column-40 league-column">
                    <div class="first-column">
                      <div class="status-icon">
                        <iframe
                          src="https://lottie.host/embed/5290d45d-a6f7-4e7f-8dec-6fd4666e4c3f/VHdTOAPywV.json"></iframe>
                      </div>
                      <span class="status-text">live</span>
                    </div>
                    <div class="middle-column">
                      <p class="time-label__content">2nd half 34'</p>
                    </div>
                    <div class="last-column">
                      <p class="league-name text-overflow">Premier League </p>
                    </div>
                  </div>
                  <div class="column-60 match-column">
                    <div class="clubs-wrapper">
                      <div class="club__item first-club">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester</p>
                        </div>
                        <img src="./images/sport-league-5.png" class="club-image" alt="">
                      </div>
                      <div class="result-score">1 - 2</div>
                      <div class="club__item second-club">
                        <img src="./images/sport-league-6.png" class="club-image" alt="">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">United</p>
                        </div>
                      </div>
                    </div>
                    <button class="btn-watch">Watch</button>
                  </div>
                </a>
								 <a class="common-section-match__item">
                  <div class="column-40 league-column">
                    <div class="first-column">
                      <div class="status-icon">
                        <iframe
                          src="https://lottie.host/embed/5290d45d-a6f7-4e7f-8dec-6fd4666e4c3f/VHdTOAPywV.json"></iframe>
                      </div>
                      <span class="status-text">live</span>
                    </div>
                    <div class="middle-column">
                      <p class="time-label__content">2nd half 34'</p>
                    </div>
                    <div class="last-column">
                      <p class="league-name text-overflow">Premier League Manchester CityManchester CityManchester City</p>
                    </div>
                  </div>
                  <div class="column-60 match-column">
                    <div class="clubs-wrapper">
                      <div class="club__item first-club">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester </p>
                        </div>
                        <img src="./images/sport-league-7.png" class="club-image" alt="">
                      </div>
                      <div class="result-score">1 - 2</div>
                      <div class="club__item second-club">
                        <img src="./images/sport-league-8.png" class="club-image" alt="">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">United Manchester City Manchester City Manchester City Manchester City Manchester City</p>
                        </div>
                      </div>
                    </div>
                    <button class="btn-watch">Watch</button>
                  </div>
                </a>
              </div>
            </div>
          </section>

					<!-- LIVE MATCH -->

					<!--HOT MATCH -->
					 <section class="common-section-match">
            <div class="common-section-match__header">
              <div class="hot-match-image">
								<iframe src="https://lottie.host/embed/6cd0ffc7-8ca7-4164-b531-03acfed1b23e/fgawmnZEYq.json"></iframe>
							</div>
              <h3 class="common-section-match__header-title hot-match__title">Hot match</h3>
            </div>
            <div class="common-section-match__body">
              <div class="common-section-match__list">
                <a class="common-section-match__item">
                  <div class="column-40 league-column">
                    <div class="first-column">
                      <span class="time-label">July 18, 02:30</span>
                    </div>
                    <div class="middle-column">

                    </div>
                    <div class="last-column">
                      <p class="league-name text-overflow">Premier League nguyen dang khoa</p>
                    </div>
                  </div>
                  <div class="column-60 match-column">
                    <div class="clubs-wrapper">
                      <div class="club__item first-club">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester United United nguyen dang khoa nguye United
                            nguyen dang khoa nguye</p>
                        </div>
                        <img src="./images/sport-league-1.png" class="club-image" alt="">
                      </div>
                      <div class="result-score">vs</div>
                      <div class="club__item second-club">
                        <img src="./images/sport-league-2.png" class="club-image" alt="">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester United nguyen dang khoa nguyen United nguyen
                            dang khoa nguyen</p>
                        </div>
                      </div>
                    </div>
                    <button class="btn-watch">Watch</button>
                  </div>
                </a>
                <a class="common-section-match__item">
                  <div class="column-40 league-column">
                    <div class="first-column">
                       <span class="time-label">July 18, 02:30</span>
                    </div>
                    <div class="middle-column">

                    </div>
                    <div class="last-column">
                      <p class="league-name text-overflow">Premier League </p>
                    </div>
                  </div>
                  <div class="column-60 match-column">
                    <div class="clubs-wrapper">
                      <div class="club__item first-club">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester</p>
                        </div>
                        <img src="./images/sport-league-1.png" class="club-image" alt="">
                      </div>
                      <div class="result-score">vs</div>
                      <div class="club__item second-club">
                        <img src="./images/sport-league-2.png" class="club-image" alt="">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester United</p>
                        </div>
                      </div>
                    </div>
                    <button class="btn-watch">Watch</button>
                  </div>
                </a>
								 <a class="common-section-match__item">
                  <div class="column-40 league-column">
                    <div class="first-column">
                       <span class="time-label">July 18, 02:30</span>
                    </div>
                    <div class="middle-column">

                    </div>
                    <div class="last-column">
                      <p class="league-name text-overflow">Premier League </p>
                    </div>
                  </div>
                  <div class="column-60 match-column">
                    <div class="clubs-wrapper">
                      <div class="club__item first-club">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester</p>
                        </div>
                        <img src="./images/sport-league-1.png" class="club-image" alt="">
                      </div>
                      <div class="result-score">vs</div>
                      <div class="club__item second-club">
                        <img src="./images/sport-league-2.png" class="club-image" alt="">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">United</p>
                        </div>
                      </div>
                    </div>
                    <button class="btn-watch">Watch</button>
                  </div>
                </a>
								 <a class="common-section-match__item">
                  <div class="column-40 league-column">
                    <div class="first-column">
                    	<span class="time-label">July 18, 02:30</span>
                    </div>
                    <div class="middle-column">

                    </div>
                    <div class="last-column">
                      <p class="league-name text-overflow">Premier League Manchester CityManchester CityManchester City</p>
                    </div>
                  </div>
                  <div class="column-60 match-column">
                    <div class="clubs-wrapper">
                      <div class="club__item first-club">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">Manchester </p>
                        </div>
                        <img src="./images/sport-league-1.png" class="club-image" alt="">
                      </div>
                      <div class="result-score">vs</div>
                      <div class="club__item second-club">
                        <img src="./images/sport-league-2.png" class="club-image" alt="">
                        <div class="club-name__wrapper">
                          <p class="club-name text-overflow">United Manchester City Manchester City Manchester City Manchester City Manchester City</p>
                        </div>
                      </div>
                    </div>
                    <button class="btn-watch">Watch</button>
                  </div>
                </a>
              </div>
            </div>
          </section>
					<!-- HOT MATCH -->

        </aside>
      </div>

    </div>
  </div>
  <?php include "templates/footer.php"; ?>
</body>

</html>